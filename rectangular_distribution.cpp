...
Vector2f randomPosition( void )
{
    ...
    random_position.x = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/W));
    random_position.y = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/H));
...
int main( void )
{
    ...
    kernel_radius = (100/(sqrt(n)+1)<10?10:100/(sqrt(n)+1));

    CircleShape kernel_circle;
    kernel_circle.setRadius(kernel_radius/5);
